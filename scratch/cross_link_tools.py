...
for filename in os.listdir(article_dir):
    if filename.endswith(".js"):
        ...
        if art_id in cross_links:
            ...
            # Convert python list to JS array string formatting
            cats_js = "[" + ", ".join(f'"{c}"' for c in cats) + "]"
            subs_js = "[" + ", ".join(f'"{s}"' for s in subs) + "]"
            
            content = cat_pattern.sub(f'categoryId: {cats_js}', content)
            content = sub_pattern.sub(f'subcategoryId: {subs_js}', content)
            
            with open(filepath, "w", encoding="utf-8") as f:
                f.write(content)
            print(f"Cross-linked JS article: {filename} -> categories: {cats_js}, subcategories: {subs_js}")
            js_updated += 1
...
for filename in os.listdir(scratch_dir):
    if filename.endswith(".py") and "build_" in filename:
        ...
        if art_id and art_id in cross_links:
            ...
            # Convert list to array strings for output
            cats_js = "[" + ", ".join(f'"{c}"' for c in cats) + "]"
            subs_js = "[" + ", ".join(f'"{s}"' for s in subs) + "]"
            ...
            with open(filepath, "w", encoding="utf-8") as f:
                f.write(content)
            print(f"Cross-linked template in build script: {filename}")
            py_updated += 1
