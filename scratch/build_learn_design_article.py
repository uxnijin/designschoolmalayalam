...
# Load parsed design learning resources
with open("scratch/learn_design_by_category.json", "r", encoding="utf-8") as f:
...
for cat_id, items in tools_data.items():
    ...
    for item in items:
        ...
        if not title:
            continue
        if not desc:
            desc = f"Curated resources to learn design with {title}."
...
print(f"Total cleaned design learning resources: {total_items}")
...
let htmlContent = `
  ...
  <p style="font-size: 1.05rem; margin-bottom: 35px; line-height: 1.85;">
    Here is a curated directory of the best <strong>""" + str(total_items) + """ resources to learn UI/UX design</strong>. Discover courses, design guidelines, reference sheets, accessibility checklists, and portfolio brief generators.
  </p>
`;
...
ARTICLE_REGISTRY["learn-ui-ux-design"] = {
  ...
  description: "Curated catalog of """ + str(total_items) + """ educational platforms to learn UI/UX design, including courses, accessibility checklists, brief generators, and talks.",
  date: "2026-06-17",
...
output_file = "article/learn-ui-ux-design.js"
